...
const parseClassFile = async arr =>
{
    ...
    // console.time('parseClassFile#' + currTimer);
    timer++;
    ...
    const readField = (fieldInfoOffset) =>
    {
        ...
        if (debug)
            console.log('>>> ' + name + ' : ' + descriptor + ', ' + accessFlags + ', isStatic: ' + ((accessFlags & Opcodes.ACC_STATIC) !== 0));

        const field = new JvmField();
        field.access = accessFlags;
        field.name = name;
        ...
        for (let i = 0; i < attributesCount; i++)
        {
            ...
            {
                let constantvalueIndex = readUnsignedShort(arr, currentOffset);
                const constantValue = constantvalueIndex === 0 ? false : readConst(constantvalueIndex);
                field.defaultValue = constantValue;
    ...
    const readMethod = (currentOffset) =>
    {
        ...
        const func = new JvmFunction();
        func.access = currentMethodAccessFlags;
        func.name = currentMethodName;
        func.desc = currentMethodDescriptor;
        if (codeOffset !== 0)
        {
            const [ insns, tryCatchBlocks ] = readCode(codeOffset, currentMethodName);

            func.insns = insns;
            func.tryCatchBlocks = tryCatchBlocks;
        }
        else
        {
            // Native?
            if (debug) console.log('native', func)
        }
        ...
        while (currentOffset < bytecodeEndOffset)
        {
            ...
            switch (opcode)
            {
                ...
                case Opcodes.SIPUSH:
                    ...
                    break;
                case Opcodes.LDC: {
                    const type = arr[cpInfoOffsets[arr[currentOffset + 1] & 0xFF] - 1];
                    insns.push({[opcode]: [type, readConst(arr[currentOffset + 1] & 0xFF)]});
                    // methodVisitor.visitLdcInsn(readConst(arr[currentOffset + 1] & 0xFF));
                    currentOffset += 2;
                } break;
                case Opcodes.LDC_W:
                case Opcodes.LDC2_W: {
                    const type = arr[cpInfoOffsets[readUnsignedShort(arr, currentOffset + 1)] - 1];
                    insns.push({[opcode]: [type, readConst(readUnsignedShort(arr, currentOffset + 1))]});
    ...
    // console.time('parseClassFile.' + thisClass);
    ...
    for (let i = 0; i < interfaces.length; i++)
    {
        ...
        if (!classes[itz])
        {
            const itzPkg = itz.substring(0, itz.lastIndexOf('/'));
            const isItzLocal = useLocalClasses && itzPkg.startsWith(localClassPkg);
            interfaces[i] = ((await loadClass(itz, isItzLocal)) ? classes[itz] : false);
        }
        else
    ...
    if (superClass)
    {
        const superPkg = superClass.substring(0, superClass.lastIndexOf('/'));
        const isSuperLocal = useLocalClasses && superPkg.startsWith(localClassPkg);
        if (!classes[superClass])
            clazz.superClass = (await loadClass(superClass, isSuperLocal)) ? classes[superClass] : false;
        else
    ...
    clazz.access = accessFlags;
    clazz.name = thisClass;
    ...
    // console.timeEnd('parseClassFile#' + currTimer);
    // console.timeEnd('parseClassFile.' + thisClass);
    return true;
}
