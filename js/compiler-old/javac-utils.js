class JavacUtils_Old {
    ...
    readWord(iter = null, doSkipWhitespace = true) {
        iter = (iter === null ? this.iter : iter);
        if (doSkipWhitespace && !this.skipWhitespace_(iter)) return false;
        let output = '';
        while (true) {
            const peek = iter.peek();
            if (!peek) {
    ...
    peekWord(iter = null) {
        iter = (iter === null ? this.iter : iter);
        const index = iter.index();
        if (!this.skipWhitespace_(iter)) return false;
        let output = '';
        while (true) {
            const peek = iter.peek();
            if (!peek) {
    ...
    skipWhitespace_(iter = null, skipComments = true) {
        iter = (iter === null ? this.iter : iter);
        while (true) {
            let peek = iter.peek();
            if (!peek) {
    ...
    readUntil_(char, iter = null, include = true) {
        iter = (iter === null ? this.iter : iter);
        let output = '';
        while (true) {
            const peek = iter.peek();
     ...
     * @param iter {Iterator}
     */
    skipUntilClosingChar_(c, iter = null) {
        iter = (iter === null ? this.iter : iter);
